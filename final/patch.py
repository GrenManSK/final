                ...
                if (fr := f.read()) == '':
                    ...
                    with open(version, 'a', encoding='utf-8') as f3:
                        if fr == '\n':
                            ...
                            os.remove(file)
                            continue
                        f3.write(name.replace(
                            'patch-temp#29', '').replace('#29', '/')[0:-4] + '\n' + fr + '\n')
    ...
    def get_patch(self, folder='temp'):
        for filename in glob.glob(folder + '/*', recursive=True):
            ...
            if os.path.isdir(filename):
                ...
                continue
            one_file(filename, filename.replace('temp\\', ''),
                     filename.replace('\\', '#29'))
...
def one_file(origin, lastest, filename):
    ...
    with open(patchFilePath, "w", encoding='utf-8') as patchFile:
        for text in patchText.split('\n'):
            text = text.replace('%22', '"').replace('%25', '%').replace('%5C', '\\').replace(
                '%5B', '[').replace('%5D', ']').replace('%7B', '{').replace('%7D', '}')
            if len(text) > 0:
                if text[0] == '+':
