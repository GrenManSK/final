...
class unpack:
    def __init__(self, default_name: str = "data.xp2", folder: str = "./", files: list = None):
        if files is None:
            files = []
    ...
    def unpack(self, download=True):
        if download:
            self.download_xp3(unpack)
        datafiles = self.files
        if len(datafiles) == 0:
            for file in os.listdir(self.folder):
                if file.startswith(self.default_name.split(".")[0]) and file.endswith("." + self.default_name.split(".")[-1]):
    ...
    def unpack_one(self, filename):
        with zipfile.ZipFile(filename, mode="r") as zip:
            for member in tqdm(
                iterable=zip.namelist(), total=len(zip.namelist()), desc="Rozbaľujem "
            ):
                with contextlib.suppress(zipfile.error):
                    zip.extract(member)
                    tqdm.write(f"{os.path.basename(member)}({str(os.path.getsize(member))}B)")
                    print(f"{os.path.basename(member)}({str(os.path.getsize(member))}B)")
                    sleep(0.01)
            zip.close()
        subprocess.call(
        ...
        for i in directories:
            ...
            if i.endswith("_cut"):
                Merge(
                    i,
                    directories[0],
                    os.path.basename(f"{i[:-4]}." + file_name.split(".")[-1]),
                ).merge(cleanup=True)
                shutil.rmtree(i)
        shutil.rmtree("data1")
        sleep(1)
