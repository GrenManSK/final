...
class pack:
    ...
    def pack_one(self):
        ...
        if datapart == 1:
            ...
            cachename = (
                self.default_name.split(".")[0]
                + "_part"
                + str(datapart)
                + "."
                + self.default_name.split(".")[-1]
            )
        sleep(0.2)
        total_size = 0
        total_size1 = 0
            ...
            for file_name in os.listdir(source_dir):
                ...
                if total_size > self.max_mbs:
                    if times == 1:
                        ...
                        os.mkdir("datafolder/" + self.filename + "/" + dir_name)
        ...
        if total_size > self.max_mbs:
            ...
            if isinstance(self.uncoded, list):
                ...
                zipfileswopath.extend([os.path.basename(i) for i in self.uncoded])
        else:
            zipfiles = ["data.xp3"]
            zipfileswopath = ["data.xp3"]
            folders = []
        ...
        with zipfile.ZipFile(cachename, mode="w", compresslevel=5) as zip:
            ...
            for i in bar:
                ...
                tqdm.write(
                    ...
                    + " KB) -> "
                    + str(round(filesize - zip_kb_old, 2))
                    + " KB"
                )
                zip_kb_old = filesize
